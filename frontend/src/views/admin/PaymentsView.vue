<template>
    <DashboardLayout>
        <!-- 添加一些过滤条件 -->
        <t-table row-key="id" :columns="columns" :data="tableData" table-layout="fixed" :loading="isLoading"
...
export default {
            ...
            columns: [
                ...
                },
                {
                    ...
                    cell: (h, { col, row }) => {
                        ...
                        let time = new Date(row.createtime * 1000 + (8 * 3600 * 1000)).toISOString()
    ...
    methods: {
        ...
        filterData(filters) {
            this.timer = setTimeout(() => {
                ...
                // 根据过滤条件过滤数据
                this.isLoading = true
                api.getOrders(this.pagination.current, this.pagination.pageSize).then((res) => {
                    let data = res.data.data
    ...
    mounted() {
        this.getPaylogs(this.pagination.defaultCurrent, this.pagination.defaultPageSize)
    }
}
</script>